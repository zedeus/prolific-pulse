...
function renderLiveStudies(studies) {
  ...
  const html = sortedStudies.map((study) => {
    ...
    const firstSeenAt = escapeHtml(study.first_seen_at ? formatRelative(study.first_seen_at) : "");
    const url = studyUrlFromId(study.id);
    const cardInner = `
      <div class="event live${isPriority ? " priority" : ""}">
        <div class="event-top">
          <div class="event-title">${name}</div>
          ${firstSeenAt ? `<div class="event-time">${firstSeenAt}</div>` : ""}
        </div>
        <div class="event-metrics">
          <span class="metric reward">${reward}</span>
          <span class="metric rate${perHourClass}">${perHour}/hr</span>
  ...
  const html = sorted.map((entry) => {
    ...
    const cardInner = `
      <div class="${cardClass}">
        ...
        <div class="event-metrics">
          <span class="metric reward">${reward}</span>
          <span class="metric rate${perHourClass}">${hourly}</span>
          <span class="metric-sep"></span>
